...
class index(UserCenterHandler.RequestHandler):
	...
	def get(self):
		if self.privilege is False:
			...
			return
		self.render_view('/account_retain.html',title='用户存留')
...
class checkFilter(UserCenterHandler.RequestHandler):
	...
	def get(self):
		...
		for retain_data in retain_datas[1]:
			new_login_accont = retain_data.get('new_login_accont',0)
			datas.append(dict(
							d_date = retain_data.get('create_time').strftime('%Y/%m/%d'),
							server = srv_datas.get(retain_data.get('s_uid','')),
							...
							regist_account = retain_data.get('regist_account',0),
							login_account = retain_data.get('login_account',0),
		...
		if len(datas)>0:
			...
			if total_[0]:
				_new_login_accont = int(total_[0].get('new_login_accont',0))
				...
				_data['new_login_accont'] = _new_login_accont
				_data['regist_account'] = int(total_[0].get('regist_account',0))
				_data['login_account'] = int(total_[0].get('login_account',0))
...
class Export(UserCenterHandler.RequestHandler):
	...
	def get(self):
		...
		if list_data[0]<1:
			...
			return
		titles = ['日期','渠道','服务器','注册账号数','登录账号数','新登账号数','次日存留率','3日存留率','4日存留率','5日存留率',
